...
{	
	...
	if("d" %in% plot.type){
		...
		if(file.output){
			if(file=="jpg")	jpeg(paste(file.prefix,".SNP_Density",".jpg",sep=""), width = 9*dpi,height=7*dpi,res=dpi,quality = 100)
			if(file=="pdf")	pdf(paste(file.prefix,".SNP_Density",".pdf",sep=""), width = 9,height=7)
			if(file=="tiff")	tiff(paste(file.prefix,".SNP_Density",".tiff",sep=""), width = 9*dpi,height=7*dpi,res=dpi)
			par(xpd=TRUE)
		}else{
		...
		if(multracks==FALSE){
			...
			for(i in 1:R){
				...
				if(verbose)	print(paste("Rectangular_Manhattan Plotting ",taxa[i],"...",sep=""))
					if(file.output){
						if(file=="jpg")	jpeg(paste(file.prefix,".Manhattan.",taxa[i],".jpg",sep=""), width = 14*dpi,height=5*dpi,res=dpi,quality = 100)
						if(file=="pdf")	pdf(paste(file.prefix,".Manhattan.",taxa[i],".pdf",sep=""), width = 15,height=6)
						if(file=="tiff")	tiff(paste(file.prefix,".Manhattan.",taxa[i],".tiff",sep=""), width = 14*dpi,height=5*dpi,res=dpi)
	...
	if("q" %in% plot.type){
		...
		if(multracks){
			...
			for(i in 1:R){
				...
				if(file.output){
					if(file=="jpg")	jpeg(paste(file.prefix,".QQplot.",taxa[i],".jpg",sep=""), width = 5.5*dpi,height=5.5*dpi,res=dpi,quality = 100)
					if(file=="pdf")	pdf(paste(file.prefix,".QQplot.",taxa[i],".pdf",sep=""), width = 5.5,height=5.5)
					if(file=="tiff")	tiff(paste(file.prefix,".QQplot.",taxa[i],".tiff",sep=""), width = 5.5*dpi,height=5.5*dpi,res=dpi)
					par(mar = c(5,5,4,2),xpd=TRUE)
				}else{
